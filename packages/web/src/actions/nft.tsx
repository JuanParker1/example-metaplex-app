...
import { Collection, DataV2 } from '@metaplex-foundation/mpl-token-metadata';
...
export const mintNFT = async (
  ...
  const tags = realFiles.reduce(
    (
      accumulator: Record<string, Array<{ name: string; value: string }>>,
      f,
    ) => {
      accumulator[f.name] = [{ name: 'mint', value: mintKey }];
      return accumulator;
    },
    {},
  );
...
export const prepPayForFilesTxn = async (
  ...
  for (const file of files) {
    const hex = await hash(await file.text());
    instructions.push(
