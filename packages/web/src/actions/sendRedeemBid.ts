...
export async function sendRedeemBid(
  ...
  if (
    ...
    const safetyDeposit = item.safetyDeposit;
    await (item.masterEdition?.info.key == MetadataKey.MasterEditionV1
      ? deprecatedSetupRedeemParticipationInstructions(
          connection,
          auctionView,
          accountsByMint,
          accountRentExempt,
...
export async function setupRedeemPrintingV2Instructions(
  ...
  for (const [index, wc] of auctionView.items.entries())
    index < winningIndex &&
      wc.forEach((index_) => {
        if (
          index_.safetyDeposit.info.order === item.safetyDeposit.info.order &&
          index_.winningConfigType === item.winningConfigType
        ) {
          offset = offset.add(index_.amount);
        }
      });

  for (let index = 0; index < item.amount.toNumber(); index++) {
    ...
    const desiredEdition = editionBase.add(offset.add(new BN(index)));
    ...
    await redeemPrintingV2Bid(
      ...
      mint,
      desiredEdition,
      new BN(offset.add(new BN(index))),
      new BN(winIndex),
      myInstructions,
    );
...
async function deprecatedSetupRedeemPrintingV1Instructions(
  ...
  if (updateAuth && auctionView.myBidderMetadata) {
    ...
    if (newTokenAccountKey && newTokenAccountBalance > 0)
      for (let index = 0; index < newTokenAccountBalance; index++) {
        console.log('Redeeming v1 token', index);
...
export async function setupRedeemParticipationInstructions(
  ...
  if (!bidRedemption?.info?.getBidRedeemed(safetyDeposit.info.order)) {
    ...
    await redeemParticipationBidV3(
      ...
      winnerIndex != undefined && winnerIndex != undefined
        ? new BN(winnerIndex)
        : null,
      myInstructions,
    );
