...
import type { SafetyDepositConfig } from '@oyster/common/dist/lib/models/metaplex/index';
...
import type { SafetyDepositDraft } from './createAuctionManager';
...
export async function addTokensToVault(
  ...
  const [vaultAuthority] = await findProgramAddress(
    [
  ...
  for (const nft of nfts) {
    if (nft.box.tokenAccount) {
      ...
      currentSigners.push(transferAuthority);
