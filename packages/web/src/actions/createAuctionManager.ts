...
export async function createAuctionManager(
  ...
  let instructions: TransactionInstruction[][] = [
    ...
    if (instr.length > 0) {
      ...
      toRemoveSigners[index] = true;
      return false;
    }
  });

  let filteredSigners = signers.filter((_, index) => !toRemoveSigners[index]);
  ...
  while (stopPoint < instructions.length && tries < 3) {
    ...
    tries = instructions.length === lastInstructionsLength ? tries + 1 : 0;
...
async function buildSafetyDepositArray(
  ...
  for (const [index, s] of safetyDeposits.entries()) {
          ...
          s.winningConfigType == WinningConfigType.FullRightsTransfer
            ...
            : new BN(
                s.amountRanges.reduce(
                  (accumulator, r) => accumulator.add(r.amount.mul(r.length)),
                  new BN(0),
                ),
              ),
      },
...
async function validateBoxes(
  ...
  for (const [index, safetyDeposit] of safetyDeposits.entries()) {
    ...
    if (
      ...
      safetyDepositBox = await getSafetyDepositBox(
        vault,
        //@ts-ignore
        safetyDeposit.draft.masterEdition.info.printingMint,
      );
    } else {
      safetyDepositBox = await getSafetyDepositBox(
        vault,
    ...
    await validateSafetyDepositBoxV2(
      ...
      safetyDepositBox,
      safetyDepositTokenStores[index],
      safetyDeposit.config.winningConfigType === WinningConfigType.PrintingV1
        ? me?.info.printingMint
        : safetyDeposit.draft.metadata.info.mint,
      wallet.publicKey.toBase58(),
...
async function deprecatedBuildAndPopulateOneTimeAuthorizationAccount(
  ...
  const signers: Keypair[] = [];
  const instructions: TransactionInstruction[] = [];
  const [recipientKey] = await findProgramAddress(
    [
      wallet.publicKey.toBuffer(),
      programIds().token.toBuffer(),
