...
import type { GetInitPackSetParameters } from './interface';

const stringToUint8Array = (string_: string, size: number): Uint8Array => {
...
export const getInitPackSet = async ({
  ...
  packSetKey,
  walletPublicKey,
}: GetInitPackSetParameters): Promise<TransactionInstruction> => {
  const {
    name,
    uri,
    description,
