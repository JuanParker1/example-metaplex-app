...
export interface ArtSelectorProps extends ButtonProps {
  ...
  filter?: (index: SafetyDepositDraft) => boolean;
}

export const ArtSelector = (properties: ArtSelectorProps) => {
  const { selected, setSelected, allowMultiple } = properties;
  let items = useUserArts();
  if (properties.filter) items = items.filter(properties.filter);
