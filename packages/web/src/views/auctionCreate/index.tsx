...
export const AuctionCreateView = () => {
  ...
  const createAuction = async () => {
    ...
    if (
      ...
      switch (attributes.category) {
        ...
        case AuctionCategory.Limited:
        case AuctionCategory.Single: {
        ...
        default: {
          ...
          for (const c of tiers)
            c.items = c.items.filter(
              (index) =>
                (index as TierDummyEntry).winningConfigType !== undefined,
            );
          const filteredTiers = tiers.filter(
            (index) => index.items.length > 0 && index.winningSpots.length > 0,
          );

          for (const [index, config] of tieredAttributes.items.entries()) {
            ...
            for (const tier of filteredTiers) {
              ...
              const item = tier.items.find(
                (index_) =>
                  (index_ as TierDummyEntry).safetyDepositBoxIndex == index,
              );
              ...
              if (item) {
                ...
                for (const [index_, spot] of sorted.entries()) {
                  if (tierRangeLookup[spot - 1]) {
                    tierRangeLookup[spot] = tierRangeLookup[spot - 1];
                    tierRangeLookup[spot].length = tierRangeLookup[
                      spot
                    ].length.add(new BN(1));
                  } else {
                    ...
                    const zeroLength =
                      index_ - 1 > 0 ? spot - sorted[index_ - 1] - 1 : spot;
                    if (zeroLength > 0) {
                      tierRanges.push(
                        new AmountRange({
                          amount: new BN(0),
    ...
    const _auctionObject = await createAuctionManager(
      connection,
      wallet,
    ...
    setAuctionObject(_auctionObject);
  };
  ...
  const congratsStep = <Congrats auction={auctionObject} />;
        ...
        <Col>
          <Row>
            <Button
              className="type-btn"
              size="large"
              onClick={() => properties.confirm(AuctionCategory.InstantSale)}
            >
              <div>
          ...
          <Row>
            <Button
              className="type-btn"
              size="large"
              onClick={() => properties.confirm(AuctionCategory.Limited)}
            >
              <div>
          ...
          <Row>
            <Button
              className="type-btn"
              size="large"
              onClick={() => properties.confirm(AuctionCategory.Open)}
            >
              <div>
          ...
          <Row>
            <Button
              className="type-btn"
              size="large"
              onClick={() => properties.confirm(AuctionCategory.Tiered)}
            >
              <div>
          ...
          <Row>
            <Button
              className="type-btn"
              size="large"
              onClick={() => properties.confirm(AuctionCategory.Single)}
            >
              <div>
...
const InstantSaleStep = ({
  ...
  setAttributes: (attribute: AuctionState) => void;
  confirm: () => void;
}) => {
                ...
                <>
                  ...
                  <Input
                    ...
                    onChange={(info) =>
                      setAttributes({
                        ...attributes,
                        editions: Number.parseInt(info.target.value),
                      })
                    }
                  />
                </>
          ...
          <label className="action-field">
            ...
            <Input
              ...
              onChange={(info) =>
                setAttributes({
                  ...attributes,
                  priceFloor: Number.parseFloat(info.target.value),
                  instantSalePrice: Number.parseFloat(info.target.value),
                })
              }
            />
          </label>
...
const CopiesStep = (properties: {
  ...
  properties.attributes.quoteMintAddress = mint
    ? mint.toBase58()
    : QUOTE_MINT.toBase58();

  if (properties.attributes.quoteMintAddress) {
    properties.attributes.quoteMintInfo = useMint(
      properties.attributes.quoteMintAddress,
    )!;
    properties.attributes.quoteMintInfoExtended = useTokenList().tokenMap.get(
      properties.attributes.quoteMintAddress,
    )!;
  }

  const artistFilter = (index: SafetyDepositDraft) =>
    !(index.metadata.info.data.creators || []).find(
      (c: Creator) => !c.verified,
    );
  let filter: (index: SafetyDepositDraft) => boolean = () => true;
  if (properties.attributes.category === AuctionCategory.Limited) {
    filter = (index: SafetyDepositDraft) =>
      !!index.masterEdition && !!index.masterEdition.info.maxSupply;
  } else if (properties.attributes.category === AuctionCategory.Open) {
    filter = (index: SafetyDepositDraft) =>
      !!(
        index.masterEdition &&
        (index.masterEdition.info.maxSupply === undefined ||
          index.masterEdition.info.maxSupply === null)
      );
  }

  const overallFilter = (index: SafetyDepositDraft) =>
    filter(index) && artistFilter(index);
      ...
      <Row className="content-action">
        <Col xl={24}>
          <ArtSelector
            ...
            selected={properties.attributes.items}
            setSelected={(items) => {
              properties.setAttributes({ ...properties.attributes, items });
            }}
            allowMultiple={false}
          >
            Select NFT
          ...
          {properties.attributes.category === AuctionCategory.Limited && (
            <label className="action-field">
...
const NumberOfWinnersStep = (properties: {
  attributes: AuctionState;
  setAttributes: (attribute: AuctionState) => void;
  confirm: () => void;
}) => {
          ...
          <label className="action-field">
            ...
            <Input
              ...
              onChange={(info) =>
                properties.setAttributes({
                  ...properties.attributes,
                  winnersCount: Number.parseInt(info.target.value),
                })
              }
            />
          </label>
...
const PriceAuction = (properties: {
  ...
  const quoteMintName =
    properties.attributes?.quoteMintInfoExtended?.name || 'Custom Token';
  const quoteMintExtension =
    properties.attributes?.quoteMintInfoExtended?.symbol ||
    shortenAddress(properties.attributes.quoteMintAddress);
  return (
    <>
      <Row className="call-to-action">
        <h2>Price</h2>
        <p>
          ...
          {properties.attributes.quoteMintAddress !=
            WRAPPED_SOL_MINT.toBase58() && (
            <a
              href={`https://explorer.solana.com/address/${properties.attributes?.quoteMintAddress}`}
              target="_blank"
              rel="noreferrer"
            >
              {' '}
              {properties.attributes?.quoteMintAddress !=
                WRAPPED_SOL_MINT.toBase58() &&
                `${quoteMintName} (${quoteMintExtension})`}
            </a>
          )}
        </p>
      </Row>
      <Row className="content-action">
        <Col className="section" xl={24}>
          {properties.attributes.category === AuctionCategory.Open && (
            <label className="action-field">
              <span className="field-title">Price</span>
              <span className="field-info">
              ...
              <Input
                ...
                prefix="◎"
                suffix={
                  properties.attributes.quoteMintInfoExtended
                    ? properties.attributes.quoteMintInfoExtended.symbol
                    : properties.attributes.quoteMintAddress ==
                      WRAPPED_SOL_MINT.toBase58()
                    ? 'SOL'
                ...
                onChange={(info) =>
                  properties.setAttributes({
                    ...
                    participationFixedPrice: Number.parseFloat(
                      info.target.value,
                    ),
                    priceFloor: Number.parseFloat(info.target.value),
                  })
          ...
          <label className="action-field">
            ...
            <Input
              ...
              onChange={(info) =>
                properties.setAttributes({
                  ...properties.attributes,
                  priceTick: Number.parseFloat(info.target.value),
                })
              }
            />
          </label>
...
const InitialPhaseStep = (properties: {
  attributes: AuctionState;
  setAttributes: (attribute: AuctionState) => void;
  confirm: () => void;
}) => {
  ...
  const [saleMoment, setSaleMoment] = useState<moment.Moment | undefined>(
    properties.attributes.startSaleTS
      ? moment.unix(properties.attributes.startSaleTS)
      : undefined,
  );
  const [listMoment, setListMoment] = useState<moment.Moment | undefined>(
    properties.attributes.startListTS
      ? moment.unix(properties.attributes.startListTS)
      : undefined,
  );
...
const EndingPhaseAuction = (properties: {
  attributes: AuctionState;
  setAttributes: (attribute: AuctionState) => void;
  confirm: () => void;
}) => {
        ...
        <Col className="section" xl={24}>
          <div className="action-field">
            ...
            <Input
              addonAfter={
                <Select
                  defaultValue={properties.attributes.auctionDurationType}
                  onChange={(value) =>
                    properties.setAttributes({
                      ...properties.attributes,
              ...
              onChange={(info) =>
                properties.setAttributes({
                  ...properties.attributes,
                  auctionDuration: Number.parseInt(info.target.value),
                })
              }
            />
          </div>
          ...
          <div className="action-field">
            ...
            <Input
              addonAfter={
                <Select
                  defaultValue={properties.attributes.gapTimeType}
                  onChange={(value) =>
                    properties.setAttributes({
                      ...properties.attributes,
                      gapTimeType: value,
              ...
              onChange={(info) =>
                properties.setAttributes({
                  ...properties.attributes,
                  gapTime: Number.parseInt(info.target.value),
                })
              }
            />
          </div>
          ...
          <label className="action-field">
            ...
            <Input
              ...
              onChange={(info) =>
                properties.setAttributes({
                  ...properties.attributes,
                  tickSizeEndingPhase: Number.parseInt(info.target.value),
                })
              }
            />
          </label>
...
const TierTableStep = (properties: {
  attributes: TieredAuctionState;
  setAttributes: (attribute: TieredAuctionState) => void;
  maxWinners: number;
  confirm: () => void;
}) => {
  ...
  for (let index = 0; index < properties.maxWinners; index++) {
    options.push({ label: `Winner ${index + 1}`, value: index });
  }
  return (
    <>
      <Row className="call-to-action">
      ...
      {properties.attributes.tiers.map((wcg, configIndex) => (
        <Row className="content-action" key={configIndex}>
          ...
          <Checkbox.Group
            options={options}
            onChange={(value) => {
              const newTiers = newImmutableTiers(properties.attributes.tiers);
              const myNewTier = newTiers[configIndex];
              myNewTier.winningSpots = value.map(
                (index) => index.valueOf() as number,
              );
          ...
          {wcg.items.map((index, itemIndex) => (
            <Col className="section" xl={8} key={itemIndex}>
              <Card>
                <ArtSelector
                  filter={artistFilter}
                  selected={
                    (index as TierDummyEntry).safetyDepositBoxIndex !==
                    undefined
                      ? [
                  ...
                  setSelected={(items) => {
                    const newItems = [
                      ...properties.attributes.items.map((it) => ({ ...it })),
                    ];

                    const newTiers = newImmutableTiers(
                      properties.attributes.tiers,
                    );
                    if (items[0]) {
                      const existing = properties.attributes.items.find(
                      ...
                      if (!othersWithSameItem) {
                        for (
                          let index_ =
                            (index as TierDummyEntry).safetyDepositBoxIndex + 1;
                          index_ < properties.attributes.items.length;
                          index_++
                        ) {
                          for (const c of newTiers)
                            for (const it of c.items) {
                              if (it.safetyDepositBoxIndex === index_)
                                it.safetyDepositBoxIndex--;
                            }
                        }
                        newItems.splice(
                ...
                {(index as TierDummyEntry).winningConfigType !== undefined && (
                  <>
                    <Select
                      defaultValue={(index as TierDummyEntry).winningConfigType}
                      style={{ width: 120 }}
                      onChange={(value) => {
                        const newTiers = newImmutableTiers(
                          properties.attributes.tiers,
                    ...
                    {((index as TierDummyEntry).winningConfigType ===
                      WinningConfigType.PrintingV1 ||
                      (index as TierDummyEntry).winningConfigType ===
                        WinningConfigType.PrintingV2) && (
                      <label className="action-field">
                        ...
                        <Input
                          ...
                          onChange={(info) => {
                            ...
                            const myNewTier =
                              newTiers[configIndex].items[itemIndex];
                            myNewTier.amount = Number.parseInt(
                              info.target.value,
                            );
                            properties.setAttributes({
                              ...properties.attributes,
...
const ParticipationStep = (properties: {
  attributes: AuctionState;
  setAttributes: (attribute: AuctionState) => void;
  confirm: () => void;
}) => {
      ...
      <Row className="content-action">
        <Col className="section" xl={24}>
          <ArtSelector
            ...
            selected={
              properties.attributes.participationNFT
                ? [properties.attributes.participationNFT]
                : []
            }
            setSelected={(items) => {
          ...
          <label className="action-field">
            ...
            <Input
              ...
              onChange={(info) =>
                properties.setAttributes({
                  ...properties.attributes,
                  participationFixedPrice: Number.parseFloat(info.target.value),
                })
              }
            />
          </label>
...
const ReviewStep = (properties: {
  ...
  const item = properties.attributes.items?.[0];

  const handleConfirm = () => {
    properties.setAttributes({
      ...properties.attributes,
      startListTS: properties.attributes.startListTS || moment().unix(),
      startSaleTS: properties.attributes.startSaleTS || moment().unix(),
    });
    properties.confirm();
  };
    ...
    <>
      ...
      <Row className="content-action">
        ...
        <Col className="section" xl={12}>
          <Statistic
            ...
            title="Copies"
            value={
              properties.attributes.editions === undefined
                ? 'Unique'
                : properties.attributes.editions
            }
          />
          {cost ? (
            <AmountLabel
      ...
      <Row style={{ display: 'block' }}>
        ...
        <Statistic
          ...
          value={
            properties.attributes.startSaleTS
              ? moment
                  .unix(properties.attributes.startSaleTS as number)
        ...
        {properties.attributes.startListTS && (
          <Statistic
            ...
            value={moment
              .unix(properties.attributes.startListTS as number)
              .format('dddd, MMMM Do YYYY, h:mm a')}
          />
        )}
        <Divider />
        <Statistic
          ...
          title="Sale ends"
          value={
            properties.attributes.endTS
              ? moment
                  .unix(properties.attributes.endTS as number)
                  .format('dddd, MMMM Do YYYY, h:mm a')
              : 'Until sold'
          }
        />
      ...
      <Row>
        <Button
          ...
          className="action-btn"
        >
          {properties.attributes.category === AuctionCategory.InstantSale
            ? 'List for Sale'
            : 'Publish Auction'}
        </Button>
        <FundsIssueModal
...
const WaitingStep = (properties: {
  ...
  useEffect(() => {
    const function_ = async () => {
      ...
      await properties.createAuction();
      clearInterval(inte);
      properties.confirm();
    };
    function_();
...
const Congrats = (properties: {
  ...
  const newTweetURL = () => {
    const parameters = {
      ...
      }/#/auction/${properties.auction?.auction.toString()}`,
