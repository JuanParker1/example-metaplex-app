...
const rerun = async ({
  ...
  setAverageBids: (number_: number) => void;
  setUsersPublished: (rec: Record<string, boolean>) => void;
  setAverageSale: (number_: number) => void;
  setHighestSale: (number_: number) => void;
  setSortedSales: (number_: number[]) => void;
  ...
  for (const auction of auctionViews) {
    ...
    if (
      ...
      if (extended && extended.info.totalUncancelledBids.toNumber() > 0) {
        ...
        const allWinningBids = bids.bids
          ...
          .map((index) => index.amount.toNumber());
        newAverageSale += allWinningBids.reduce(
    ...
    let type: AuctionType | undefined;
    if (auction.items.find((set) => set.length > 1)) {
      type = AuctionType.Tiered;
    } else if (auction.items.length > 0 && auction.items[0].length > 0) {
      type =
  ...
  for (const acct of Object.values(bidderPotsByAuctionAndBidder)) {
  ...
  for (const acct of metadata) {
    newBuild[acct.info.updateAuthority] = true;
    existingUsersEngaged[acct.info.updateAuthority] = true;
    if (acct.info.data.creators)
      for (const c of acct.info.data.creators) {
...
const MemoizedBar = React.memo(function BarImpl(properties: {
  sortedSales: number[];
  mint: MintInfo;
}) {
  ...
  const asArray = [0, 5, 20, 50, 100, 500, 1000, 10_000];

  for (let index = 0; index < asArray.length; index++) {
    const currentRange = asArray[index];

    if (index < asArray.length - 1) {
      const nextRange = asArray[index + 1];
      histogrammedData[currentRange] = properties.sortedSales.filter(
        (s) =>
          fromLamports(s, properties.mint) >= currentRange &&
          fromLamports(s, properties.mint) < nextRange,
      ).length;
    } else {
      histogrammedData[currentRange] = properties.sortedSales.filter(
        (s) => fromLamports(s, properties.mint) >= currentRange,
      ).length;
    }
  }
...
const MemoizedPie = React.memo(function PieImpl(properties: {
    ...
    datasets: [
      {
        label: '#',
        data: [
          properties.byType[AuctionType.Open],
          properties.byType[AuctionType.Limited],
          properties.byType[AuctionType.Tiered],
          properties.byType[AuctionType.OneOfKind],
        ],
        backgroundColor: [
          'rgba(255, 99, 132, 0.2)',
          'rgba(54, 162, 235, 0.2)',
    ...
    <Content>
      <Col style={{ marginTop: 10 }}>
        ...
        <h3>
          Total Sales: ◎
          {fromLamports(
            sortedSales.reduce((accumulator, r) => (accumulator += r), 0),
            mint,
          )}
        </h3>
