...
import type {
  GetProvingProcessParameters,
  RequestCardsUsingVoucherParameters,
} from '../interface';
...
export const getProvingProcess = async ({
  ...
  connection,
  wallet,
}: GetProvingProcessParameters): Promise<ParsedAccount<ProvingProcess>> => {
...
const requestCardsUsingVoucher = async ({
  ...
  connection,
  wallet,
}: RequestCardsUsingVoucherParameters): Promise<
