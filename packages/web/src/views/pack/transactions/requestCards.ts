...
import type {
  RequestCardsParameters,
  RequestCardsInstructionsParameters,
  RequestCardParameters,
} from './interface';
...
export const requestCards = async ({
  ...
  editionKey,
  editionMint,
}: RequestCardsParameters): Promise<number> => {
...
const getRequestCardsInstructions = async ({
  ...
  tokenAccount,
  ...parameters
}: RequestCardsInstructionsParameters): Promise<TransactionInstruction[][]> => {
...
const generateRequestCardInstructions = async ({
  ...
  wallet,
  ...parameters
}: RequestCardParameters): Promise<TransactionInstruction[]> => {
  if (!wallet.publicKey) throw new WalletNotConnectedError();
