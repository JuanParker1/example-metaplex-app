...
import { AuctionState, createPipelineExecutor } from '@oyster/common';
...
export function useCachedRedemptionKeysByWallet() {
  ...
  useEffect(() => {
    if (!publicKey) return;
    (async () => {
      const temporary: CachedRedemptionKeys = Object.assign(
        {},
      ...
      if (Object.keys(temporary).length > 0) {
        setCachedRedemptionKeys(temporary);
      }
    })();
...
function isInstantSale(
  auctionDataExtension: ParsedAccount<AuctionDataExtended> | null,
  auction: ParsedAccount<AuctionData>,
) {
  return !!(
    auctionDataExtension?.info.instantSalePrice &&
    auction.info.priceFloor.minPrice &&
    auctionDataExtension?.info.instantSalePrice.eq(
      auction.info.priceFloor.minPrice,
    )
  );
}
...
export function buildListWhileNonZero<T>(hash: Record<string, T>, key: string) {
  ...
  if (ticket) {
    ...
    let index = 1;
    while (ticket) {
      ticket = hash[key + '-' + index.toString()];
...
export function processAccountsIntoAuctionView(
  ...
  if (auctionManagerInstance) {
    ...
    const auctionDataExtendedKey =
      auctionManagerInstance.info.key == MetaplexKey.AuctionManagerV2
        ? (auctionManagerInstance as ParsedAccount<AuctionManagerV2>).info
            .auctionDataExtended
        : null;
    const auctionDataExtension = auctionDataExtendedKey
    ...
    const view: Partial<AuctionView> = {
      ...
      state,
      vault,
      auctionDataExtended: auctionDataExtension || undefined,
    ...
    view.isInstantSale = isInstantSale(auctionDataExtension, auction);
