...
export const uint64 = (property = 'uint64'): unknown => {
  ...
  layout.decode = (buffer: Buffer, offset: number) => {
    ...
    return new BN(
      [...data]
        .reverse()
        .map((index) => `00${index.toString(16)}`.slice(-2))
        .join(''),
      16,
    );
  };

  layout.encode = (number_: BN, buffer: Buffer, offset: number) => {
    const a = number_.toArray().reverse();
    let b = Buffer.from(a);
    if (b.length !== 8) {
...
export const rustString = (property = 'string'): unknown => {
  ...
  rsl.encode = (string_: string, buffer: Buffer, offset: number) => {
    const data = {
      chars: Buffer.from(string_, 'utf8'),
    };
