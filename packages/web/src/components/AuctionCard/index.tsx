...
import type { BidRedemptionTicket } from '@oyster/common/dist/lib/models/metaplex/index';
...
async function calculateTotalCostOfRedeemingOtherPeoplesBids(
  ...
  for (let index = 0; index < max; index++) {
    const winner = auctionView.auction.info.bidState.getWinnerAt(index);
    if (!winner) {
      break;
    } else {
      ...
      if (bid) {
        for (
          let index_ = 0;
          index_ <
          auctionView.auctionManager.safetyDepositBoxesExpected.toNumber();
          index_++
        ) {
          totalWinnerItems += auctionView.auctionManager
            .getAmountForWinner(index, index_)
            .toNumber();
        }
      }
    }
  ...
  return !!useMemo(() => {
    if (gapTick && value && gapTime && !auctionView.auction.info.ended()) {
      ...
      if (endedAt) {
        ...
        if (now > ended) {
          const toLamportValue = value * LAMPORTS_PER_MINT;
          ...
          for (
            let index = auctionView.auction.info.bidState.bids.length - 1;
            index > -1;
            index--
          ) {
            const bid = auctionView.auction.info.bidState.bids[index];
            const expected = bid.amount.toNumber();
            if (expected < toLamportValue) {
              const higherExpectedAmount = expected * ((100 + gapTick) / 100);

              return higherExpectedAmount > toLamportValue;
            } else if (expected === toLamportValue) {
...
export const AuctionCard = ({
  ...
  const minBid =
    ...
    (isUpcoming || bids.length === 0
      ...
      ? Number.parseFloat(formatTokenAmount(bids[0].info.lastBid, mintInfo))
      : 9_999_999) +
  ...
  const endInstantSale = async () => {
    ...
    try {
      ...
      console.error('endAuction', error);
      setLoading(false);
      return;
    }
  ...
  const instantSale = async () => {
    ...
    if (instantSalePrice && (allowBidToPublic || allowBidToAuctionOwner)) {
      try {
        ...
        console.error('sendPlaceBid', error);
        setLoading(false);
        return;
      }
    }
              ...
              <Button
                ...
                disabled={
                  ...
                  (!auctionView.myBidderMetadata &&
                    isAuctionManagerAuthorityNotWalletOwner) ||
                  loading ||
                  !!auctionView.items.find((index) =>
                    index.find((it) => !it.metadata),
                  )
                }
                onClick={async () => {
                  setLoading(true);
