...
export function useSettlementAuctions({
            ...
            if (
              ((balance.value.uiAmount || 0) === 0 &&
                av.auction.info.bidState.bids
                  ...
                  .reduce((accumulator, r) => (accumulator += r), 0) > 0) ||
              ...
              (balance.value.uiAmount || 0) > 0.000_01
            ) {
  ...
  for (const auctionViewKey of Object.keys(validDiscoveredEndedAuctions)) {
    ...
    if (
      ...
      validDiscoveredEndedAuctions[auctionViewKey] > 0
    )
      notifications.push({
        id: auctionViewKey,
...
export function Notifications() {
  ...
  for (const v of vaultsNeedUnwinding) {
    notifications.push({
      id: v.pubkey,
  ...
  for (const v of possiblyBrokenAuctionManagerSetups.filter(
    (v) => v.auctionManager.authority === walletPubkey,
  )) {
    notifications.push({
  ...
  for (const m of metaNeedsApproving) {
    notifications.push({
      id: m.pubkey,
  ...
  for (const v of upcomingAuctions.filter(
    (v) => v.auctionManager.authority === walletPubkey,
  )) {
    notifications.push({
      id: v.auctionManager.pubkey,
