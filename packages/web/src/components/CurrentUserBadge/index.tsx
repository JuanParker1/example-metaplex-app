...
const UserActions = (properties: { mobile?: boolean; onClick?: any }) => {
        ...
        (properties.mobile ? (
          <div className="actions-buttons actions-user">
            {canCreate && (
              <Link to={`/art/create`}>
                <Button
                  onClick={() => {
                    properties.onClick ? properties.onClick() : null;
                  }}
                  className="black-btn"
                >
                  Create
          ...
          >
            ...
            <Link to={`/auction/create/0`} style={{ width: '100%' }}>
              <Button className="metaplex-button-default" style={buttonStyle}>
                Sell
              </Button>
            </Link>
          </div>
...
const AddFundsModal = (properties: {
  ...
  return (
    <MetaplexModal
      visible={properties.showAddFundsModal}
      onCancel={() => properties.setShowAddFundsModal(false)}
      title="Add Funds"
      bodyStyle={{
        alignItems: 'start',
      }}
    >
      <div style={{ maxWidth: '100%' }}>
        ...
        >
          ...
          <span>
            {formatNumber.format(properties.balance)}&nbsp;&nbsp;
            <span
              style={{
                borderRadius: '50%',
                background: 'black',
        ...
        <Button
          onClick={() => properties.setShowAddFundsModal(false)}
          style={{
            background: '#454545',
            borderRadius: 14,
            width: '30%',
        ...
        <Button
          onClick={() => {
            window.open(
              `https://ftx.com/pay/request?coin=SOL&address=${properties.publicKey?.toBase58()}&tag=&wallet=sol&memoIsRequired=false`,
              '_blank',
...
export const CurrentUserBadge = (properties: {
  showBalance?: boolean;
  showAddress?: boolean;
  iconSize?: number;
}) => {
  ...
  let name = properties.showAddress ? shortenAddress(`${publicKey}`) : '';
  const unknownWallet = wallet as any;
  if (unknownWallet.name && !properties.showAddress) {
...
export const Cog = () => {
  ...
  const routerSearchParameters = useQuerySearch();
          ...
          >
            ...
            <Select
              onSelect={(network) => {
                ...
                routerSearchParameters.set('network', network);
                const nextLocationHash = `${
                  windowHash.split('?')[0]
                }?${routerSearchParameters.toString()}`;
...
export const CurrentUserBadgeMobile = (properties: {
  showBalance?: boolean;
  showAddress?: boolean;
  iconSize?: number;
  closeModal?: any;
    ...
    <div className="current-user-mobile-badge">
      ...
      <div className="actions-buttons">
        <Button
          ...
          onClick={() => {
            properties.closeModal ? properties.closeModal() : null;
