...
export const ArtMinting = ({ id, onMint }: ArtMintingProps) => {
  ...
  const mintingDestinationError = useMemo(() => {
  ...
  const isMintingDisabled =
    isLoading || editions < 1 || Boolean(mintingDestinationError);

  const debouncedEditionsChangeHandler = useCallback(
    debounce((value) => {
      setEditions(value < 1 ? 1 : value);
    }, 300),
    [],
  );
        ...
        <div>
          ...
          >
            <Form.Item
              ...
              labelAlign="left"
              colon={false}
              validateStatus={mintingDestinationError ? 'error' : 'success'}
              help={mintingDestinationError}
            >
              <Input
