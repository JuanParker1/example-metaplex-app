...
import type { MetadataCategory, MetadataFile } from '@oyster/common';
...
const VideoArtContent = ({
  ...
  const playerReference = useCallback(
    (reference) => {
      setPlayerApi(reference);
    },
    [setPlayerApi],
  );
  ...
  const likelyVideo = (files || []).find((f, index, array) => {
    if (typeof f !== 'string') {
      return false;
    }

    // TODO: filter by fileType
    return array.length >= 2 ? index === 1 : index === 0;
  }) as string;
  ...
  const content =
    ...
    likelyVideo.startsWith('https://watch.videodelivery.net/') ? (
      <div className={`${className} square`}>
        <Stream
          // @ts-ignore
          streamRef={(e: any) => playerReference(e)}
...
export const ArtContent = ({
  ...
  const animationUrlExtension = new URLSearchParams(
  ...
  if (
    allowMeshRender &&
    (categoryState === 'vr' ||
      animationUrlExtension === 'glb' ||
      animationUrlExtension === 'gltf')
  ) {
    return (
      <MeshArtContent
        uri={uriState}
  ...
  if (categoryState === 'html' || animationUrlExtension === 'html') {
    return (
      <HTMLContent
        uri={uriState}
        animationUrl={animationURLState}
